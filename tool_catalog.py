...
TOOL_INFO = {
    ...
    'Assign projection': {
        ...
        'relations': {
            ...
            'Common CRS Errors': 'This algorithm writes a coordinate reference system definition onto a layer whose CRS is missing or wrong. Coordinates themselves are untouched, so nothing is reprojected; only the declared system changes. Reach for it when a Shapefile arrives without a .prj file or carries an incorrect identifier, then reproject afterwards if different coordinates are needed. An incorrect assignment sends features to the wrong part of the world, so the original system must genuinely be known.',
        },
    },
    'AWS': {
    ...
    'Buffer': {
        ...
        'relations': {
            ...
            'Infrastructure Planning': 'Buffer builds polygons around input features at a fixed distance or a distance read from an attribute field. Segment count controls how smoothly curves are approximated, while end cap style, join style, and mitre limit shape line endings and corners. Negative distances shrink polygons, and the dissolve option merges overlapping buffers into single features. Distance is measured in layer units, so projected coordinates are normally required before metric results mean anything.',
            'Overlay': 'Buffer builds polygons around input features at a fixed distance or a distance read from an attribute field. Segment count controls how smoothly curves are approximated, while end cap style, join style, and mitre limit shape line endings and corners. Negative distances shrink polygons, and the dissolve option merges overlapping buffers into single features. Distance is measured in layer units, so projected coordinates are normally required before metric results mean anything.',
            'Site Selection': 'Buffer builds polygons around input features at a fixed distance or a distance read from an attribute field. Segment count controls how smoothly curves are approximated, while end cap style, join style, and mitre limit shape line endings and corners. Negative distances shrink polygons, and the dissolve option merges overlapping buffers into single features. Distance is measured in layer units, so projected coordinates are normally required before metric results mean anything.',
    ...
    'Change Detection': {
        ...
        'blurb': 'Change detection is a workflow rather than a single button: two or more coregistered dates of imagery or vector data are compared to establish what altered between them. Common routes include raster differencing or ratioing of indices such as NDVI, post-classification comparison of thematic maps, and spectral change vector analysis. Radiometric and geometric alignment matter, since misregistration and illumination differences manufacture false change. Thresholds separating real change from noise need validation against reference samples.',
        'relations': {
        },
    },
    'Channel network': {
    ...
    'Clip': {
        ...
        'relations': {
            ...
            'Overlay': 'Clip cuts an input layer to the boundary of an overlay polygon layer, keeping only the portions falling inside. Geometry type of the input is preserved, so lines stay lines and polygons stay polygons, and attributes come from the input alone. Unlike intersection, no fields from the clip layer appear in the result. Both layers should share a coordinate reference system, and the overlay is dissolved internally to avoid duplicated output parts.',
            'Site Selection': 'Clip cuts an input layer to the boundary of an overlay polygon layer, keeping only the portions falling inside. Geometry type of the input is preserved, so lines stay lines and polygons stay polygons, and attributes come from the input alone. Unlike intersection, no fields from the clip layer appear in the result. Both layers should share a coordinate reference system, and the overlay is dissolved internally to avoid duplicated output parts.',
            'Vector Analysis': 'Clip cuts an input layer to the boundary of an overlay polygon layer, keeping only the portions falling inside. Geometry type of the input is preserved, so lines stay lines and polygons stay polygons, and attributes come from the input alone. Unlike intersection, no fields from the clip layer appear in the result. Both layers should share a coordinate reference system, and the overlay is dissolved internally to avoid duplicated output parts.',
            'Vector Processing': 'Clip cuts an input layer to the boundary of an overlay polygon layer, keeping only the portions falling inside. Geometry type of the input is preserved, so lines stay lines and polygons stay polygons, and attributes come from the input alone. Unlike intersection, no fields from the clip layer appear in the result. Both layers should share a coordinate reference system, and the overlay is dissolved internally to avoid duplicated output parts.',
        },
    },
    ...
    'Difference': {
        ...
        'relations': {
            ...
            'Vector Processing': 'Difference erases the parts of input features covered by an overlay layer, returning only what remains. Attributes come from the input layer alone and geometry type is preserved. Order matters: swapping input and overlay produces a different answer, and symmetrical difference is a separate algorithm. Invalid geometries in either layer are a frequent cause of failure, so repairing them beforehand is prudent; both layers should share the same coordinate reference system.',
        },
    },
    'Dissolve': {
        'source': 'QGIS / ArcGIS Processing',
        ...
        'relations': {
            ...
            'Vector Analysis': 'Features merge into single geometries, either combining everything or grouping by one or more attribute fields. Shared boundaries between merged polygons disappear, while non-adjacent members become multipart features. Output attributes are taken from the first feature of each group unless statistics are computed separately, so summing populations needs an explicit aggregation step. Slivers and unsnapped edges can leave hairline gaps that survive the merge and show up in later overlays.',
            'Vector Processing': 'Features merge into single geometries, either combining everything or grouping by one or more attribute fields. Shared boundaries between merged polygons disappear, while non-adjacent members become multipart features. Output attributes are taken from the first feature of each group unless statistics are computed separately, so summing populations needs an explicit aggregation step. Slivers and unsnapped edges can leave hairline gaps that survive the merge and show up in later overlays.',
        },
    },
    'Distance Matrix': {
        'source': 'QGIS Processing',
    ...
    'gdal_fillnodata': {
        ...
        'relations': {
            ...
            'Raster Processing': 'Gaps in a raster are filled by interpolating from the valid pixels around each hole, using inverse distance weighting along the hole margins followed by optional smoothing iterations. A maximum search distance in pixels limits how far interpolation reaches, leaving larger voids untouched, and an explicit mask band can define what counts as missing. Filled values are estimates, so voids in elevation or index rasters should be documented rather than silently trusted.',
        },
    },
    'gdal_grid': {
        'source': 'GDAL',
    ...
    'gdal_merge': {
        ...
        'relations': {
            ...
            'Raster Processing': 'Several rasters are mosaicked into one output file, with later inputs painted over earlier ones where they overlap. Options set output resolution, extent, nodata handling, initialisation value, and output type, and a separate mode stacks each input as its own band instead of blending them spatially. All inputs must share a coordinate system, since no reprojection happens. For large mosaics a virtual raster plus warping is usually faster.',
        },
    },
    'gdal_polygonize': {
        'source': 'GDAL',
    ...
    'gdal_proximity': {
        ...
        'relations': {
            ...
            'Raster Analysis': 'A distance raster is produced where every cell records how far it lies from the nearest target pixel. Targets are defined by a list of values, or by any non-zero pixel when no list is given, and distances are reported in georeferenced units or pixels. A maximum distance caps the computation, with cells beyond it taking a nodata or fixed value. Buffer zone modelling and cost surface preparation both draw on it.',
            'Raster Processing': 'A distance raster is produced where every cell records how far it lies from the nearest target pixel. Targets are defined by a list of values, or by any non-zero pixel when no list is given, and distances are reported in georeferenced units or pixels. A maximum distance caps the computation, with cells beyond it taking a nodata or fixed value. Buffer zone modelling and cost surface preparation both draw on it.',
        },
    },
    'gdal_retile': {
        'source': 'GDAL',
    ...
    'gdal_translate': {
        ...
        'relations': {
            ...
            'GIS Files': 'Rasters are converted between formats while optionally subsetting and adjusting them. A source window in pixels or a projected window crops the extent, band selection reorders or drops bands, output size or resolution resamples, and scaling maps input value ranges onto a new data type. Creation options control compression, tiling, and predictors, including COG output. Metadata, nodata, and colour tables can be set or stripped during the copy.',
    ...
    'gdal_viewshed': {
        ...
        'relations': {
            ...
            'Visibility Analysis': 'Visibility from a single observer is computed over a raster elevation band. Observer x and y coordinates, height above the surface, optional target height, and a maximum distance define the analysis, and a curvature coefficient accounts for earth curvature and atmospheric refraction. Output is a raster coding visible, invisible, and out-of-range cells with configurable values, or, in DEM and ground modes, the minimum height needed for visibility.',
        },
    },
    'gdaladdo': {
        'source': 'GDAL',
    ...
    'gdaldem slope': {
        ...
        'relations': {
            ...
            'Terrain Analysis': 'Slope mode computes steepness from an elevation band, reporting degrees by default or percent rise when requested. A scale factor reconciles vertical and horizontal units, which is mandatory for elevation in metres stored in a geographic coordinate system, and the algorithm can be Horn or Zevenbergen-Thorne, the latter often preferred for smooth surfaces. Nodata cells propagate, and the computation reads a three by three neighbourhood around each pixel.',
        },
    },
    'gdaldem TPI': {
        'source': 'GDAL',
    ...
    'gdalinfo': {
        ...
        'relations': {
            ...
            'Metadata': 'Metadata for a raster dataset is reported: driver, pixel dimensions, coordinate reference system in WKT or PROJ form, geotransform, corner coordinates in both projected and geographic terms, band data types, block sizes, nodata values, colour interpretation, and available overviews. Additional switches compute minimum and maximum, full statistics, or histograms, and a JSON option produces machine-readable output. Running it first answers most questions before any processing is attempted.',
            'Metadata Validation': 'Metadata for a raster dataset is reported: driver, pixel dimensions, coordinate reference system in WKT or PROJ form, geotransform, corner coordinates in both projected and geographic terms, band data types, block sizes, nodata values, colour interpretation, and available overviews. Additional switches compute minimum and maximum, full statistics, or histograms, and a JSON option produces machine-readable output. Running it first answers most questions before any processing is attempted.',
            'Precision': 'Metadata for a raster dataset is reported: driver, pixel dimensions, coordinate reference system in WKT or PROJ form, geotransform, corner coordinates in both projected and geographic terms, band data types, block sizes, nodata values, colour interpretation, and available overviews. Additional switches compute minimum and maximum, full statistics, or histograms, and a JSON option produces machine-readable output. Running it first answers most questions before any processing is attempted.',
            'Raster': 'Metadata for a raster dataset is reported: driver, pixel dimensions, coordinate reference system in WKT or PROJ form, geotransform, corner coordinates in both projected and geographic terms, band data types, block sizes, nodata values, colour interpretation, and available overviews. Additional switches compute minimum and maximum, full statistics, or histograms, and a JSON option produces machine-readable output. Running it first answers most questions before any processing is attempted.',
            'Units': 'Metadata for a raster dataset is reported: driver, pixel dimensions, coordinate reference system in WKT or PROJ form, geotransform, corner coordinates in both projected and geographic terms, band data types, block sizes, nodata values, colour interpretation, and available overviews. Additional switches compute minimum and maximum, full statistics, or histograms, and a JSON option produces machine-readable output. Running it first answers most questions before any processing is attempted.',
            'Vertical CRS': 'Metadata for a raster dataset is reported: driver, pixel dimensions, coordinate reference system in WKT or PROJ form, geotransform, corner coordinates in both projected and geographic terms, band data types, block sizes, nodata values, colour interpretation, and available overviews. Additional switches compute minimum and maximum, full statistics, or histograms, and a JSON option produces machine-readable output. Running it first answers most questions before any processing is attempted.',
        },
    },
    'gdalwarp': {
        'source': 'GDAL',
        ...
        'relations': {
            ...
            'Change Detection': 'Warping covers reprojection, resampling, mosaicking, and clipping in one utility. Source and target coordinate systems, target resolution or dimensions, output extent, and resampling method from nearest through bilinear, cubic, lanczos, average, and mode are all specified as options. A cutline vector can mask output, optionally cropping to its extent, and destination nodata governs edge fill. Multithreading and warp memory settings materially affect speed on large inputs.',
            'Common CRS Errors': 'Warping covers reprojection, resampling, mosaicking, and clipping in one utility. Source and target coordinate systems, target resolution or dimensions, output extent, and resampling method from nearest through bilinear, cubic, lanczos, average, and mode are all specified as options. A cutline vector can mask output, optionally cropping to its extent, and destination nodata governs edge fill. Multithreading and warp memory settings materially affect speed on large inputs.',
            'Data Conversion': 'Warping covers reprojection, resampling, mosaicking, and clipping in one utility. Source and target coordinate systems, target resolution or dimensions, output extent, and resampling method from nearest through bilinear, cubic, lanczos, average, and mode are all specified as options. A cutline vector can mask output, optionally cropping to its extent, and destination nodata governs edge fill. Multithreading and warp memory settings materially affect speed on large inputs.',
            'Datums': 'Warping covers reprojection, resampling, mosaicking, and clipping in one utility. Source and target coordinate systems, target resolution or dimensions, output extent, and resampling method from nearest through bilinear, cubic, lanczos, average, and mode are all specified as options. A cutline vector can mask output, optionally cropping to its extent, and destination nodata governs edge fill. Multithreading and warp memory settings materially affect speed on large inputs.',
            'GIS Files': 'Warping covers reprojection, resampling, mosaicking, and clipping in one utility. Source and target coordinate systems, target resolution or dimensions, output extent, and resampling method from nearest through bilinear, cubic, lanczos, average, and mode are all specified as options. A cutline vector can mask output, optionally cropping to its extent, and destination nodata governs edge fill. Multithreading and warp memory settings materially affect speed on large inputs.',
            'Geographic': 'Warping covers reprojection, resampling, mosaicking, and clipping in one utility. Source and target coordinate systems, target resolution or dimensions, output extent, and resampling method from nearest through bilinear, cubic, lanczos, average, and mode are all specified as options. A cutline vector can mask output, optionally cropping to its extent, and destination nodata governs edge fill. Multithreading and warp memory settings materially affect speed on large inputs.',
    ...
    'geometry.is_valid': {
        'source': 'Shapely',
        'blurb': 'This property reports whether a geometry satisfies validity rules such as properly closed rings, correctly nested interior rings, and absence of self-intersections. It returns a simple boolean per geometry, evaluated by GEOS, and vectorised access over a GeoSeries flags an entire layer at once. Because invalid input can make overlay and area operations fail or return wrong answers, checking is a sensible first step; companion functions explain the reason and attempt repair.',
        'relations': {
        },
    },
    ...
    'geopandas.read_postgis()': {
        ...
        'relations': {
            ...
            'Schema Validation': 'A SQL query is executed against PostGIS and the result returned as a GeoDataFrame. The geometry column name is supplied through geom_col, a connection comes from SQLAlchemy or psycopg, and crs can be stated explicitly when the query strips SRID information. Because arbitrary SQL is allowed, joins, filters, and spatial predicates can run in the database where indexes live, reducing the volume transferred into Python.',
            'Vector': 'A SQL query is executed against PostGIS and the result returned as a GeoDataFrame. The geometry column name is supplied through geom_col, a connection comes from SQLAlchemy or psycopg, and crs can be stated explicitly when the query strips SRID information. Because arbitrary SQL is allowed, joins, filters, and spatial predicates can run in the database where indexes live, reducing the volume transferred into Python.',
        },
    },
    'geopandas.set_crs()': {
        'source': 'GeoPandas',
        ...
        'relations': {
            ...
            'Common CRS Errors': 'A coordinate reference system is attached to a GeoDataFrame whose crs is missing, describing what the existing coordinates already mean. No coordinate values change, so this is metadata assignment rather than transformation, and to_crs must be used when different coordinates are wanted. Overwriting a CRS that is already set requires allow_override, a guard against accidental mislabelling. Choosing the wrong code here corrupts every later measurement and reprojection.',
        },
    },
    'geopandas.sjoin()': {
        'source': 'GeoPandas',
    ...
    'geopandas.to_crs()': {
        ...
        'relations': {
            ...
            'Common CRS Errors': 'Geometries are transformed into a target coordinate reference system given as a CRS object, EPSG code, or authority string, with PROJ applying the necessary datum shift. The frame must already carry a valid crs, since the source definition drives the pipeline; without one, set_crs comes first. Choosing an appropriate projected system before computing area, length, or buffers is what makes those measurements defensible over regional extents.',
            'Geographic': 'Geometries are transformed into a target coordinate reference system given as a CRS object, EPSG code, or authority string, with PROJ applying the necessary datum shift. The frame must already carry a valid crs, since the source definition drives the pipeline; without one, set_crs comes first. Choosing an appropriate projected system before computing area, length, or buffers is what makes those measurements defensible over regional extents.',
            'Units': 'Geometries are transformed into a target coordinate reference system given as a CRS object, EPSG code, or authority string, with PROJ applying the necessary datum shift. The frame must already carry a valid crs, since the source definition drives the pipeline; without one, set_crs comes first. Choosing an appropriate projected system before computing area, length, or buffers is what makes those measurements defensible over regional extents.',
        },
    },
    'geopandas.to_file()': {
        'source': 'GeoPandas',
        ...
        'relations': {
            ...
            'Tables': 'A GeoDataFrame is written to disk through OGR, with the driver inferred from the file extension or stated explicitly. GeoPackage handles long field names, multiple layers, and mixed types comfortably, whereas Shapefile truncates field names to ten characters, limits field types, and splits into several sidecar files. A layer argument names the target table and mode allows appending. The active CRS is recorded in the output automatically.',
            'Vector': 'A GeoDataFrame is written to disk through OGR, with the driver inferred from the file extension or stated explicitly. GeoPackage handles long field names, multiple layers, and mixed types comfortably, whereas Shapefile truncates field names to ten characters, limits field types, and splits into several sidecar files. A layer argument names the target table and mode allows appending. The active CRS is recorded in the output automatically.',
        },
    },
    'geopandas.to_parquet()': {
        ...
        'relations': {
            ...
            'Tables': 'Vector data is saved as GeoParquet, embedding geometry column metadata and the coordinate reference system alongside columnar values. Compression codec, row group size, index handling, and schema version are configurable, and geometry is encoded as WKB by default. Reading back with read_parquet restores geometries and CRS without a translation layer. Column pruning and predicate pushdown make the format markedly faster than GeoJSON or Shapefile for large tables.',
            'Vector': 'Vector data is saved as GeoParquet, embedding geometry column metadata and the coordinate reference system alongside columnar values. Compression codec, row group size, index handling, and schema version are configurable, and geometry is encoded as WKB by default. Reading back with read_parquet restores geometries and CRS without a translation layer. Column pruning and predicate pushdown make the format markedly faster than GeoJSON or Shapefile for large tables.',
        },
    },
    ...
    'Heatmap / kernel density estimation': {
        'source': 'QGIS / spatial stats',
        'blurb': 'Kernel density estimation spreads each observation across its surroundings with a decaying weight, then sums contributions onto a grid to give a smooth intensity surface. Bandwidth, or search radius, governs how much smoothing happens and matters far more than the kernel shape selected. Resulting values represent events per unit area, so cell size and units must be reported for sensible interpretation. Edge effects and unsuitable projections distort estimates near study boundaries.',
        'relations': {
        },
    },
    'Hillshade': {
    ...
    'Intersection': {
        ...
        'relations': {
            ...
            'Overlay': 'Intersection keeps only the geometric overlap between an input layer and an overlay layer, and the output carries attributes from both. Features that do not overlap disappear entirely, and output geometry can be lower dimension than the input, for instance lines where polygons meet. Matching coordinate reference systems are expected, and invalid geometries commonly cause failures. Overlay work of this kind multiplies feature counts, so results are often dissolved afterwards.',
            'Site Selection': 'Intersection keeps only the geometric overlap between an input layer and an overlay layer, and the output carries attributes from both. Features that do not overlap disappear entirely, and output geometry can be lower dimension than the input, for instance lines where polygons meet. Matching coordinate reference systems are expected, and invalid geometries commonly cause failures. Overlay work of this kind multiplies feature counts, so results are often dissolved afterwards.',
            'Urban Growth Analysis': 'Intersection keeps only the geometric overlap between an input layer and an overlay layer, and the output carries attributes from both. Features that do not overlap disappear entirely, and output geometry can be lower dimension than the input, for instance lines where polygons meet. Matching coordinate reference systems are expected, and invalid geometries commonly cause failures. Overlay work of this kind multiplies feature counts, so results are often dissolved afterwards.',
            'Vector Analysis': 'Intersection keeps only the geometric overlap between an input layer and an overlay layer, and the output carries attributes from both. Features that do not overlap disappear entirely, and output geometry can be lower dimension than the input, for instance lines where polygons meet. Matching coordinate reference systems are expected, and invalid geometries commonly cause failures. Overlay work of this kind multiplies feature counts, so results are often dissolved afterwards.',
    ...
    'Join attributes by nearest': {
        ...
        'relations': {
            ...
            'Vector Analysis': 'Rather than requiring overlap, this algorithm matches each input feature to its closest features in a second layer. A maximum number of neighbours and an optional maximum distance bound the search, and the output gains the joined attributes plus distance, feature index, and nearest-point coordinates. Unmatched inputs may be kept or dropped. Distances follow layer units, so a projected coordinate reference system keeps thresholds meaningful.',
    ...
    'Kernel Density Estimation': {
        ...
        'blurb': 'Density estimation converts discrete observations into a continuous surface by placing a decaying kernel over each point and summing them across a grid. Bandwidth choice governs the level of detail, kernel shape has a milder effect, and optional weights let events carry different importance. Output cells report intensity per unit area, so pixel size and CRS units must accompany any interpretation. Boundary regions are underestimated unless edge correction is applied.',
        'relations': {
        },
    },
    'Leaflet': {
    ...
    'Line intersections': {
        ...
        'relations': {
            ...
            'Vector Analysis': 'Where lines from two layers cross, this algorithm creates point features at the crossing locations. Attributes from both input line layers are carried into the result, optionally restricted to selected fields, which makes it easy to see which pairs of lines meet. Shared endpoints and overlapping collinear segments behave differently from clean crossings, so topology deserves attention. Road and rail conflict detection, and stream crossing inventories, are common applications.',
        },
    },
    'MapLibre GL JS': {
        'source': 'MapLibre',
    ...
    'ogr2ogr': {
        ...
        'relations': {
            ...
            'Tables': 'Vector data is converted between OGR formats with substantial transformation along the way. Options reproject between coordinate systems, apply attribute filters or full SQL statements, clip to a spatial extent or cutline layer, rename layers, force a geometry type, select or reorder fields, and pass driver-specific creation options. Append and overwrite modes support incremental loading, including into PostGIS, making it a standard tool in ETL scripting.',
            'Temporal Data': 'Vector data is converted between OGR formats with substantial transformation along the way. Options reproject between coordinate systems, apply attribute filters or full SQL statements, clip to a spatial extent or cutline layer, rename layers, force a geometry type, select or reorder fields, and pass driver-specific creation options. Append and overwrite modes support incremental loading, including into PostGIS, making it a standard tool in ETL scripting.',
            'Vector': 'Vector data is converted between OGR formats with substantial transformation along the way. Options reproject between coordinate systems, apply attribute filters or full SQL statements, clip to a spatial extent or cutline layer, rename layers, force a geometry type, select or reorder fields, and pass driver-specific creation options. Append and overwrite modes support incremental loading, including into PostGIS, making it a standard tool in ETL scripting.',
        },
    },
    'ogrinfo': {
        'source': 'GDAL/OGR',
        ...
        'relations': {
            ...
            'Metadata': 'Vector datasets are inspected without modification. Output lists available layers and, for each, the geometry type, feature count, extent, coordinate reference system, and field definitions with types and widths. A summary-only switch avoids dumping every feature, while SQL execution allows ad hoc queries against the source and a JSON mode produces structured output. Running it before conversion reveals encoding, schema, and CRS surprises that would otherwise break a pipeline.',
            'Metadata Validation': 'Vector datasets are inspected without modification. Output lists available layers and, for each, the geometry type, feature count, extent, coordinate reference system, and field definitions with types and widths. A summary-only switch avoids dumping every feature, while SQL execution allows ad hoc queries against the source and a JSON mode produces structured output. Running it before conversion reveals encoding, schema, and CRS surprises that would otherwise break a pipeline.',
            'Schema Validation': 'Vector datasets are inspected without modification. Output lists available layers and, for each, the geometry type, feature count, extent, coordinate reference system, and field definitions with types and widths. A summary-only switch avoids dumping every feature, while SQL execution allows ad hoc queries against the source and a JSON mode produces structured output. Running it before conversion reveals encoding, schema, and CRS surprises that would otherwise break a pipeline.',
            'Units': 'Vector datasets are inspected without modification. Output lists available layers and, for each, the geometry type, feature count, extent, coordinate reference system, and field definitions with types and widths. A summary-only switch avoids dumping every feature, while SQL execution allows ad hoc queries against the source and a JSON mode produces structured output. Running it before conversion reveals encoding, schema, and CRS surprises that would otherwise break a pipeline.',
            'Vector': 'Vector datasets are inspected without modification. Output lists available layers and, for each, the geometry type, feature count, extent, coordinate reference system, and field definitions with types and widths. A summary-only switch avoids dumping every feature, while SQL execution allows ad hoc queries against the source and a JSON mode produces structured output. Running it before conversion reveals encoding, schema, and CRS surprises that would otherwise break a pipeline.',
            'Vertical CRS': 'Vector datasets are inspected without modification. Output lists available layers and, for each, the geometry type, feature count, extent, coordinate reference system, and field definitions with types and widths. A summary-only switch avoids dumping every feature, while SQL execution allows ad hoc queries against the source and a JSON mode produces structured output. Running it before conversion reveals encoding, schema, and CRS surprises that would otherwise break a pipeline.',
        },
    },
    'OpenAI API': {
        'source': 'OpenAI',
        'blurb': 'The OpenAI API exposes hosted language, embedding, image, and audio models over HTTP, authenticated with an API key. Chat completions accept messages and return generated text, structured outputs can be constrained by a schema, and function or tool calling lets a model request external actions. Embeddings turn text into vectors for similarity search. In geospatial work it typically assists with metadata summarisation, code generation, and text classification, not with computing geometry.',
        'relations': {
        },
    },
    'OpenDroneMap': {
    ...
    'openrouteservice isochrones': {
        ...
        'relations': {
            ...
            'Catchment Analysis': 'Reachability polygons are generated around one or more locations for a given profile, using either time or distance ranges. Multiple range values in a single request produce nested bands, an interval argument subdivides them evenly, and location_type switches between departing from and arriving at the points. Optional attributes report polygon area, reachable population, or total road length. Smoothing controls how tightly the boundary follows the underlying network.',
        },
    },
    ...
    'pgr_aStar': {
        ...
        'relations': {
            ...
            'Network Analysis': 'A* finds a least-cost path while guiding the search toward the destination with a geometric heuristic, which usually visits fewer nodes than plain Dijkstra. The edges query must supply node coordinates as x1, y1, x2, y2 so the heuristic can estimate remaining distance, and options select among several heuristic formulations plus factor and epsilon parameters. Factor must align heuristic units with cost units, otherwise the result may not be optimal.',
            'Routing': 'A* finds a least-cost path while guiding the search toward the destination with a geometric heuristic, which usually visits fewer nodes than plain Dijkstra. The edges query must supply node coordinates as x1, y1, x2, y2 so the heuristic can estimate remaining distance, and options select among several heuristic formulations plus factor and epsilon parameters. Factor must align heuristic units with cost units, otherwise the result may not be optimal.',
            'Trajectory': 'A* finds a least-cost path while guiding the search toward the destination with a geometric heuristic, which usually visits fewer nodes than plain Dijkstra. The edges query must supply node coordinates as x1, y1, x2, y2 so the heuristic can estimate remaining distance, and options select among several heuristic formulations plus factor and epsilon parameters. Factor must align heuristic units with cost units, otherwise the result may not be optimal.',
            'Trajectory Analysis': 'A* finds a least-cost path while guiding the search toward the destination with a geometric heuristic, which usually visits fewer nodes than plain Dijkstra. The edges query must supply node coordinates as x1, y1, x2, y2 so the heuristic can estimate remaining distance, and options select among several heuristic formulations plus factor and epsilon parameters. Factor must align heuristic units with cost units, otherwise the result may not be optimal.',
        },
    },
    'pgr_dijkstra': {
        'source': 'pgRouting',
        ...
        'relations': {
            ...
            'Network': 'Shortest paths are computed over a graph defined by an SQL query returning edge identifier, source, target, cost, and reverse cost. One-to-one, one-to-many, many-to-one, many-to-many, and combination forms exist, and a directed flag decides whether reverse cost is honoured. Output rows give sequence, path identifier, node, edge, cost, and cumulative cost, with the final edge marked minus one. Negative reverse cost marks an edge as impassable in that direction.',
            'Network Analysis': 'Shortest paths are computed over a graph defined by an SQL query returning edge identifier, source, target, cost, and reverse cost. One-to-one, one-to-many, many-to-one, many-to-many, and combination forms exist, and a directed flag decides whether reverse cost is honoured. Output rows give sequence, path identifier, node, edge, cost, and cumulative cost, with the final edge marked minus one. Negative reverse cost marks an edge as impassable in that direction.',
            'Routing': 'Shortest paths are computed over a graph defined by an SQL query returning edge identifier, source, target, cost, and reverse cost. One-to-one, one-to-many, many-to-one, many-to-many, and combination forms exist, and a directed flag decides whether reverse cost is honoured. Output rows give sequence, path identifier, node, edge, cost, and cumulative cost, with the final edge marked minus one. Negative reverse cost marks an edge as impassable in that direction.',
            'Trajectory': 'Shortest paths are computed over a graph defined by an SQL query returning edge identifier, source, target, cost, and reverse cost. One-to-one, one-to-many, many-to-one, many-to-many, and combination forms exist, and a directed flag decides whether reverse cost is honoured. Output rows give sequence, path identifier, node, edge, cost, and cumulative cost, with the final edge marked minus one. Negative reverse cost marks an edge as impassable in that direction.',
            'Trajectory Analysis': 'Shortest paths are computed over a graph defined by an SQL query returning edge identifier, source, target, cost, and reverse cost. One-to-one, one-to-many, many-to-one, many-to-many, and combination forms exist, and a directed flag decides whether reverse cost is honoured. Output rows give sequence, path identifier, node, edge, cost, and cumulative cost, with the final edge marked minus one. Negative reverse cost marks an edge as impassable in that direction.',
        },
    },
    ...
    'pgr_dijkstraVia': {
        ...
        'relations': {
            ...
            'Trajectory': 'A route visiting a list of vertices in the given order is computed as a sequence of Dijkstra legs stitched together. Output rows identify the path segment, its start and end vertices, per-edge cost, aggregate cost within the leg, and the running total for the whole journey, so each stage can be styled separately. Strict and U-turn options control behaviour when a leg cannot be completed or must reverse at a stop.',
            'Trajectory Analysis': 'A route visiting a list of vertices in the given order is computed as a sequence of Dijkstra legs stitched together. Output rows identify the path segment, its start and end vertices, per-edge cost, aggregate cost within the leg, and the running total for the whole journey, so each stage can be styled separately. Strict and U-turn options control behaviour when a leg cannot be completed or must reverse at a stop.',
        },
    },
    'pgr_drivingDistance': {
        'source': 'pgRouting',
        ...
        'relations': {
            ...
            'Catchment Analysis': 'All nodes reachable from one or more start vertices within a given cost limit are returned, each with the aggregate cost of reaching it. The output is a set of vertices rather than a polygon, so an alpha shape or convex hull step is needed to draw a service area boundary. Directedness and an equicost option, which assigns shared nodes to their cheapest source, control behaviour with multiple starting points.',
            'Connectivity': 'All nodes reachable from one or more start vertices within a given cost limit are returned, each with the aggregate cost of reaching it. The output is a set of vertices rather than a polygon, so an alpha shape or convex hull step is needed to draw a service area boundary. Directedness and an equicost option, which assigns shared nodes to their cheapest source, control behaviour with multiple starting points.',
            'Network': 'All nodes reachable from one or more start vertices within a given cost limit are returned, each with the aggregate cost of reaching it. The output is a set of vertices rather than a polygon, so an alpha shape or convex hull step is needed to draw a service area boundary. Directedness and an equicost option, which assigns shared nodes to their cheapest source, control behaviour with multiple starting points.',
            'Network Analysis': 'All nodes reachable from one or more start vertices within a given cost limit are returned, each with the aggregate cost of reaching it. The output is a set of vertices rather than a polygon, so an alpha shape or convex hull step is needed to draw a service area boundary. Directedness and an equicost option, which assigns shared nodes to their cheapest source, control behaviour with multiple starting points.',
        },
    },
    'pgr_KSP': {
    ...
    'pyproj.CRS': {
        ...
        'relations': {
            ...
            'CRS Validation': 'A coordinate reference system is represented as an object built from an EPSG code, WKT, PROJ string, PROJJSON, or authority name. Properties expose axis order and units, the datum and ellipsoid, whether the system is geographic, projected, or compound, and its declared area of use. Equality comparison and to_epsg lookups help reconcile definitions that differ textually but describe the same system. It underpins CRS handling in GeoPandas and Rasterio.',
            'Datums': 'A coordinate reference system is represented as an object built from an EPSG code, WKT, PROJ string, PROJJSON, or authority name. Properties expose axis order and units, the datum and ellipsoid, whether the system is geographic, projected, or compound, and its declared area of use. Equality comparison and to_epsg lookups help reconcile definitions that differ textually but describe the same system. It underpins CRS handling in GeoPandas and Rasterio.',
            'Geographic': 'A coordinate reference system is represented as an object built from an EPSG code, WKT, PROJ string, PROJJSON, or authority name. Properties expose axis order and units, the datum and ellipsoid, whether the system is geographic, projected, or compound, and its declared area of use. Equality comparison and to_epsg lookups help reconcile definitions that differ textually but describe the same system. It underpins CRS handling in GeoPandas and Rasterio.',
            'Units': 'A coordinate reference system is represented as an object built from an EPSG code, WKT, PROJ string, PROJJSON, or authority name. Properties expose axis order and units, the datum and ellipsoid, whether the system is geographic, projected, or compound, and its declared area of use. Equality comparison and to_epsg lookups help reconcile definitions that differ textually but describe the same system. It underpins CRS handling in GeoPandas and Rasterio.',
            'Vertical CRS': 'A coordinate reference system is represented as an object built from an EPSG code, WKT, PROJ string, PROJJSON, or authority name. Properties expose axis order and units, the datum and ellipsoid, whether the system is geographic, projected, or compound, and its declared area of use. Equality comparison and to_epsg lookups help reconcile definitions that differ textually but describe the same system. It underpins CRS handling in GeoPandas and Rasterio.',
        },
    },
    'pyproj.Transformer': {
        'source': 'PyProj',
        ...
        'relations': {
            ...
            'Datums': 'Transformer objects convert coordinates between two coordinate reference systems, created most often with from_crs and then reused for many points, since construction is the expensive part. An area of interest can steer selection toward a more accurate operation, and available pipelines with their accuracies can be listed before choosing. Arrays are handled efficiently, and by default axis order follows each CRS definition rather than always being longitude first.',
            'Geographic': 'Transformer objects convert coordinates between two coordinate reference systems, created most often with from_crs and then reused for many points, since construction is the expensive part. An area of interest can steer selection toward a more accurate operation, and available pipelines with their accuracies can be listed before choosing. Arrays are handled efficiently, and by default axis order follows each CRS definition rather than always being longitude first.',
            'Vertical CRS': 'Transformer objects convert coordinates between two coordinate reference systems, created most often with from_crs and then reused for many points, since construction is the expensive part. An area of interest can steer selection toward a more accurate operation, and available pipelines with their accuracies can be listed before choosing. Arrays are handled efficiently, and by default axis order follows each CRS definition rather than always being longitude first.',
        },
    },
    ...
    'QGIS Server': {
        ...
        'relations': {
            ...
            'Web Mapping': 'QGIS Server publishes QGIS projects as OGC web services, including WMS, WFS, WCS, WMTS, and OGC API Features. Because it uses the same rendering engine as the desktop, symbology, labelling, and print layouts carry through unchanged, and layouts become printable templates via GetPrint. Deployment runs as FastCGI behind a web server or in a container, with project settings controlling which layers are published and their metadata.',
        },
    },
    'QGIS Style Manager': {
        'source': 'QGIS',
    ...
    'Raster Calculator': {
        ...
        'relations': {
            ...
            'Suitability Analysis': 'Cell-by-cell expressions are evaluated across one or more raster bands to produce a new raster. Operators cover arithmetic, comparison, and logic, and functions include trigonometric, logarithmic, and conditional forms; band references combine the layer name with a band index. Output extent, cell size, and CRS are set explicitly, and inputs are expected to align. Nodata cells propagate, which quietly shrinks coverage when several layers are combined.',
    ...
    'rasterio.features.rasterize()': {
        'source': 'Rasterio',
        'blurb': 'Vector geometries are burned into a raster array. Pairs of geometry and value, or a constant burn value, are written according to a supplied output shape and affine transform, with fill setting the background. The all_touched flag switches from pixel-centre inclusion to touching any part of a cell, and a merge algorithm decides how overlapping geometries combine. Output dtype must accommodate the burn values, and the counterpart shapes function reverses the process.',
        'relations': {
        },
    },
    'rasterio.mask.mask()': {
    ...
    'Simplify': {
        ...
        'relations': {
            ...
            'Vector Processing': 'Vertices are removed to reduce geometric detail while retaining overall shape. Method choices include distance-based Douglas-Peucker, snapping to a grid, and area-based Visvalingam simplification, each governed by a tolerance in layer units. Higher tolerances yield smaller files and faster rendering at the cost of positional fidelity. Neighbouring polygons are processed independently, so shared boundaries can separate or overlap, and self-intersections may appear, making a validity check afterwards worthwhile.',
        },
    },
    'Slope': {
        'source': 'QGIS / GDAL DEM',
    ...
    'ST_Buffer': {
        ...
        'relations': {
            ...
            'Spatial SQL': 'A geometry covering every point within a given distance of the input is returned. Optional style parameters set quadrant segments, end cap as round, flat, or square, join as round, mitre, or bevel, and side for one-sided line buffers. Negative distances erode polygons and can eliminate narrow ones entirely. Geometry inputs measure distance in spatial reference units, while geography inputs work in metres, which usually explains surprising results in degree-based data.',
        },
    },
    'ST_ClusterDBSCAN': {
        'source': 'PostGIS',
    ...
    'ST_Contains': {
        ...
        'relations': {
            ...
            'Spatial SQL': 'ST_Contains reports whether the second geometry lies entirely inside the first, requiring that no point of B falls outside A and that their interiors share at least one point. A consequence of that interior rule is that a polygon does not contain a line lying wholly along its boundary, where ST_Covers would return true. Bounding-box indexing accelerates the test, and argument order is the reverse of ST_Within.',
        },
    },
    'ST_Crosses': {
        'source': 'PostGIS / Spatial SQL',
    ...
    'ST_DWithin': {
        ...
        'relations': {
            ...
            'Infrastructure Planning': 'ST_DWithin answers whether two geometries lie within a specified distance of each other, returning true or false without computing the exact separation. Because it can be rewritten to use a bounding-box index expanded by the distance, it performs far better than comparing ST_Distance to a threshold. Geometry arguments use spatial reference units while geography arguments use metres, and geography accepts a flag to trade spheroid accuracy for sphere speed.',
    ...
    'ST_Intersects': {
        ...
        'relations': {
            ...
            'Hex Grid': 'ST_Intersects returns true whenever two geometries share any portion of space, making it the complement of ST_Disjoint and the most widely used spatial predicate. Query planners combine it with the bounding-box overlap operator so a GiST index prunes candidates before exact geometry comparison. Both arguments must share an SRID, otherwise the call errors. Geography arguments are supported, evaluating intersection on the spheroid rather than on a plane.',
            'Spatial SQL': 'ST_Intersects returns true whenever two geometries share any portion of space, making it the complement of ST_Disjoint and the most widely used spatial predicate. Query planners combine it with the bounding-box overlap operator so a GiST index prunes candidates before exact geometry comparison. Both arguments must share an SRID, otherwise the call errors. Geography arguments are supported, evaluating intersection on the spheroid rather than on a plane.',
        },
    },
    'ST_IsValid': {
        'source': 'PostGIS / Spatial SQL',
        ...
        'relations': {
            ...
            'Validity': 'Validity of a geometry is tested against OGC rules covering closed rings, correctly nested holes, absence of self-intersections, and no repeated or degenerate segments. A false result signals that overlay and measurement functions may fail or return nonsense, so validation belongs early in any loading routine. Companion functions explain the problem in text or return its location, and ST_MakeValid attempts repair, sometimes changing geometry type in the process.',
        },
    },
    'ST_IsValidDetail': {
        'source': 'PostGIS / Spatial SQL',
    ...
    'ST_Transform': {
        ...
        'relations': {
            ...
            'Common CRS Errors': 'Coordinates are reprojected into a target spatial reference identifier, applying the datum shift and projection maths through PROJ. The input geometry must already carry a correct SRID, since the source definition drives the transformation; variants accept explicit PROJ pipelines or authority-qualified codes when a specific operation is required. Transforming inside a query prevents index use unless the expression is indexed, so storing a projected column often performs better.',
            'Database Processing': 'Coordinates are reprojected into a target spatial reference identifier, applying the datum shift and projection maths through PROJ. The input geometry must already carry a correct SRID, since the source definition drives the transformation; variants accept explicit PROJ pipelines or authority-qualified codes when a specific operation is required. Transforming inside a query prevents index use unless the expression is indexed, so storing a projected column often performs better.',
            'Datums': 'Coordinates are reprojected into a target spatial reference identifier, applying the datum shift and projection maths through PROJ. The input geometry must already carry a correct SRID, since the source definition drives the transformation; variants accept explicit PROJ pipelines or authority-qualified codes when a specific operation is required. Transforming inside a query prevents index use unless the expression is indexed, so storing a projected column often performs better.',
            'Geographic': 'Coordinates are reprojected into a target spatial reference identifier, applying the datum shift and projection maths through PROJ. The input geometry must already carry a correct SRID, since the source definition drives the transformation; variants accept explicit PROJ pipelines or authority-qualified codes when a specific operation is required. Transforming inside a query prevents index use unless the expression is indexed, so storing a projected column often performs better.',
            'Spatial SQL': 'Coordinates are reprojected into a target spatial reference identifier, applying the datum shift and projection maths through PROJ. The input geometry must already carry a correct SRID, since the source definition drives the transformation; variants accept explicit PROJ pipelines or authority-qualified codes when a specific operation is required. Transforming inside a query prevents index use unless the expression is indexed, so storing a projected column often performs better.',
            'Units': 'Coordinates are reprojected into a target spatial reference identifier, applying the datum shift and projection maths through PROJ. The input geometry must already carry a correct SRID, since the source definition drives the transformation; variants accept explicit PROJ pipelines or authority-qualified codes when a specific operation is required. Transforming inside a query prevents index use unless the expression is indexed, so storing a projected column often performs better.',
            'Vertical CRS': 'Coordinates are reprojected into a target spatial reference identifier, applying the datum shift and projection maths through PROJ. The input geometry must already carry a correct SRID, since the source definition drives the transformation; variants accept explicit PROJ pipelines or authority-qualified codes when a specific operation is required. Transforming inside a query prevents index use unless the expression is indexed, so storing a projected column often performs better.',
        },
    },
    'ST_Within': {
        'source': 'PostGIS / Spatial SQL',
